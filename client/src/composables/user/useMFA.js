...
export function useMFA() {
  ...
  const setupMFA = async (code) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/auth/setup-mfa?code=${encodeURIComponent(code)}`, {
        method: 'POST',
        headers: {
  ...
  const verifyMFA = async (email, code) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/auth/verify-mfa?email=${encodeURIComponent(email)}&code=${encodeURIComponent(code)}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
